...
def ransac(data, model, n, k, t, d, debug=False, return_all=False):
    ...
    while iterations < k:
        ...
        maybe_inliers = data[maybe_idxs, :]  # 内群点坐标
        test_points = data[test_idxs]  # (Xi,Yi)数据点
        maybemodel = model.fit(maybe_inliers)  # 拟合模型
        test_err = model.get_error(test_points, maybemodel)  # 计算平方和误差
        also_idxs = test_idxs[test_err < t]                  #其余点平方和误差小于t的记作内群点，获取索引值
...
def test():
    ...
    B_noisy = B_exact + \
              np.random.normal(size=B_exact.shape)  # 500 * 1行向量,代表Yi
